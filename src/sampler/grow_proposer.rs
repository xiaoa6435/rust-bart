        ...
        self.tree
            ...
            .for_each(|(idx, node_ptr)| {
                if *node_ptr == cur_idx && new.go_left(&self.data_para.x, idx) {
                    ret.get_mut(&left_idx).unwrap().update(idx, self.resid, w);
                    *node_ptr = left_idx;
                } else if *node_ptr == cur_idx {
                    ret.get_mut(&right_idx).unwrap().update(idx, self.resid, w);
                    *node_ptr = right_idx;
                } else {
                    ret.get_mut(&node_ptr).unwrap().update(idx, self.resid, w)
                }
            });
